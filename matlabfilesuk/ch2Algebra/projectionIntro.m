...
projlen = norm(hatx); % abs(x'*v)/(norm(v))
...
% the text from overlapping the vector 
dx=[-0.1 -0.4 0];
t1=t*v(1)+dx(1); t2=t*v(2)+dx(2); t3=t*v(3)+dx(3);
text(t1,t2,t3, ['$\hat x=tv=' num2str(t) '*v$'], ...
    'FontSize',fs,'Rotation',-5, 'Interpreter','latex');

quiver3(x(1),x(2),x(3),-x(1)+hatx(1),-x(2)+hatx(2),-x(3)+hatx(3),0,'r')
dy=0.15;
text(x(1),x(2),x(3)+dy,'$x-\hat x$','FontSize',fs, ...
    'Rotation',-57,'Interpreter','latex')
